...
/*
	 ...
	 * name:保存;
	 * tips:保存;
	 * params:[{id:'closed',name:'是否关闭当前页面',value:'true'},{id:'parameter',name:'页面参数',value:''},{id:'isexcel',name:'页面参数',value:''},{id:'showTooltip',name:'是否弹出提示框',value:'true'},{id:'okCallbackFun',name:'保存成功回调函数',value:''},{id:'okCallbackFunParams',name:'保存成功回调函数参数',value:''}];
	 * imageSrc:/clientrest?op=resource&path=/css/images/page_white_acrobat.png;
	 * description:保存页面数据;
 */
  function save(closed,parameter,isexcel,showTooltip,okCallbackFun,okCallbackFunParams){
	  if(!validateAll()){
		  return false;
	  }
	  var paramsJson ={};
	...
	if(__excel){
		if(__excel.dataSetMapping){
			var subData=__excel.getSubmitData();//["submitData"];
			var formIdArr = [];
			var gridIdArr = [];
	...
	var url = clientpath+"/clientrest?op=Save";
	for(var dsId in formData){
		var fd=formData[dsId];
		for(var flag in fd){
			//insertData updateData
			var fdm=fd[flag][0];
			for(var arr in fdm){
				if (typeof(fdm[arr]) == "undefined"){
					fdm[arr]="";
					continue;
				}
				if (!fdm[arr] && typeof(fdm[arr])!="undefined" && fdm[arr]!=0){ 
					fdm[arr]="";
					continue;
				}
				if(fdm[arr]){
					fdm[arr]=encodeURIComponent(encodeURIComponent(fdm[arr]));
				}				
			}
		}
	}
	var formDataStr=JSON.encode(formData);
	var formDataStr=escape(formDataStr);
	var params = "sceneId=" + sceneId + "&formData=" + formDataStr + 
	"&gridData=" + encodeURIComponent(encodeURIComponent(JSON.encode(gridData))) + "&pageId=" + pageId + "&params=" +encodeURIComponent(encodeURIComponent(JSON.encode(paramsJson)));
	var a = new Ajax(url,params);
	var r = false;
	a.success = function(result){
		if(__excel){
			//更新列表数据状态为load
			__excel.refreshGridStatus();
		}
		...
		if(typeof okCallbackFun === "function" ){
			okCallbackFun(okCallbackFunParams);
		}
	};
 ...
 */
  function formLoad(formIds,parameter){
		...
		var url = clientpath+"/clientrest?op=FormLoad";
		var forms = document.forms;
		for(var i =0;i<forms.length;i++){
		    var formId = forms[i].getAttribute("id");
...
function formInit(formIds,parameter){
		...
		if(!formIds){
			...
			var url = clientpath+"/clientrest?op=FormInit";
			for(var i=0;i<datasetIdArr.length;i++){
				var datasetId=datasetIdArr[i];
				pageData[datasetId]=[];
...
function pageLoad(parameter){
	...
	//列表的分页信息传递结束
	var url = clientpath+"/clientrest?op=PageLoad";
	pt.ui.ajax({
		type:"post",
		url:url,
		data:{
		...
		success : function(result){
			...
			for(var i=0;i<gridDatasetIdArr.length;i++){
				...
				for(var attr in gridParamsJson){
	   			...
	   			grid.basicParam = gridParamsJson;
	   			grid.url = clientpath+"/clientrest?op=GridLoad";
...
function gridLoad(gridIds,parameter){
	...
	var url = clientpath+"/clientrest?op=GridLoad";
	var gridArr = null;
...
function  setPageData(formdataStr){
 ...
 //权限校验
 //@param permissionsValidation
function permissionsValidate(permissionsValidation){
	var roleStr=window.params.role;
	if(roleStr==undefined||roleStr==""){
		return;
	}
	...
	//permissionsValidation={'admin':[{'PTUITextField_31':'hidden'}]};//{'admin':[{'PTUIButton_16':'disabled'},{'PTUIDateTime_20':'readonly'},{'PTUITextField_23':'disabled'},{'PTUITextField_26':'readonly'}]};
		...
		if(role==roleStr){
			...
			for(var i=0;i<permArr.length;i++){
				...
				for(var per in perm){
					...
					if(widgetPer=="READONLY"||widgetPer=="readonly"){
						$("#"+widgetId).attr("readonly",true);
					}
					if(widgetPer=="DISABLED"||widgetPer=="disabled"){
